...
> (5) 对于任意 $n$ 阶实对称半正定矩阵 $M$, 存在矩阵 $A$ 使得 $M=A^{\mathsf T}A$ 成立. 
  ...
  $$
  \Lambda^{\frac{1}{2}} = \mathrm{diag} (\sqrt{\lambda\_1}, \dots, \sqrt{\lambda_n})
  $$
  ...
  $$\begin{aligned}  M &= Q\Lambda Q^{\mathsf T} \\\\ 
  &= (\Lambda^{\frac{1}{2}}Q^{\mathsf T})^{\mathsf T}\Lambda^{\frac{1}{2}}Q^{\mathsf T} \\\\
